...
export default function PackagesManagementPage() {
  ...
  const { cities, categories } = useCities();

  const categoryList =
    categories && categories.length > 0
      ? categories.map((c) => c.name)
      : ['North India', 'South India', 'Japan', 'America', 'Europe'];
  ...
  const handleCityChange = (cityName) => {
    ...
    const matchedCat = matchedCity?.categoryName || categories[0]?.name || 'North India';
    setFormData((prev) => ({
      ...prev,
      cityName,
      category: matchedCat,
      categoryTag: matchedCat,
    }));
  };
  ...
  const openAddModal = () => {
    ...
    setActiveTab('basic');
    const defaultCity = selectedCityFilter !== 'All' ? selectedCityFilter : (cities[0]?.name || 'Jodhpur');
    const matchedCity = cities.find((c) => c.name === defaultCity);
    const defaultCategory = matchedCity?.categoryName || categories[0]?.name || 'North India';
  ...
  const openEditModal = (pkg) => {
    ...
    setActiveTab('basic');
    const matchedCity = cities.find((c) => c.name === pkg.cityName);
    const resolvedCat = pkg.category || matchedCity?.categoryName || pkg.categoryTag || 'North India';
  ...
  const filteredPackages = tours.filter((pkg) => {
    ...
    const matchesTag =
      ...
      pkg.categoryTag === selectedTagFilter ||
      pkg.category === selectedTagFilter;
    const matchesSearch =
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredPackages.map((pkg) => {
          const matchedCity = cities.find((c) => c.name === pkg.cityName || c.id === pkg.cityId);
          const categoryDisplay = matchedCity?.categoryName || pkg.category || pkg.categoryTag;
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      >
                        {cities.map((c) => (
                          <option key={c.id} value={c.name}>
                            {c.name} ({c.categoryName || 'Destination'})
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        value={formData.categoryTag || formData.category}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
